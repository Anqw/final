...
class FastRCNNOutputs(object):
    ...
    def __init__(
        self, box2box_transform, pred_class_logits, pred_proposal_deltas, pred_cooperative_logits, proposals, smooth_l1_beta
    ):
        """
        Args:
        ...
        self.pred_cooperative_logits = pred_cooperative_logits
        self.smooth_l1_beta = smooth_l1_beta
        self.coop_loss = torch.nn.CosineEmbeddingLoss(reduction = "mean")
    ...
    def cooperating_loss(self):
        """
        Compute the softmax cross entropy loss for indirect classification.

        Returns:
            scalar Tensor
        """
        bg_class_ind = self.pred_cooperative_logits.shape[1] - 1
        fg_inds = torch.nonzero((self.gt_classes >= 0) & (self.gt_classes < bg_class_ind)).squeeze(1)
        return F.cross_entropy(self.pred_cooperative_logits[fg_inds], self.gt_classes[fg_inds], reduction="mean")
    ...
    def losses(self):
        ...
        """
        if self.pred_cooperative_logits is not None:
            return {
                "loss_cls": self.softmax_cross_entropy_loss(),
                "loss_box_reg": self.smooth_l1_loss(),
                "loss_cooperating": 0.1*self.cooperating_loss(),
            }
        else:
            return {
                "loss_cls": self.softmax_cross_entropy_loss(),
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x_cls, x_reg):
        ...
        if x_reg.dim() > 2:
            x_reg = torch.flatten(x_cls, start_dim=1)
        scores = self.cls_score(x_cls)
        proposal_deltas = self.bbox_pred(x_reg)
        scores_cls = None
        return scores, proposal_deltas, scores_cls
...
class CosineSimOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        if self.meta:
            self.num_base = int((num_classes/4) * 3)
            self.device = torch.device(cfg.MODEL.DEVICE)
            base_weight = cfg.MODEL.BASE
            self.ckpt = torch.load(base_weight, map_location=torch.device('cpu'))
    ...
    def forward(self, x_cls, x_reg):
        ...
        w = self.cls_score.weight
        cos_dist = self.similarity_f(x_cls.unsqueeze(1), w.unsqueeze(0))

        if self.meta:
            b = self.ckpt['model']['roi_heads.box_predictor.cls_score.weight'].to(self.device)
            query_base_dist = self.similarity_f(x_cls.unsqueeze(1), b.unsqueeze(0))
            novel_base_dist = self.similarity_f(w.unsqueeze(1), b.unsqueeze(0))

            scores_novel = self.similarity_f(query_base_dist[:, :self.num_base].unsqueeze(1), novel_base_dist[:, :self.num_base].unsqueeze(0))
            scores_new = 15.0 * scores_novel
            scores = self.scale * cos_dist
        else:
            scores_new = None
            scores = self.scale * cos_dist
        proposal_deltas = self.bbox_pred(x_reg)
        return scores, proposal_deltas, scores_new