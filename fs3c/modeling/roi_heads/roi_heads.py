...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        bbox_cls_feats, bbox_reg_feats = self.box_head(box_features)
        pred_class_logits, pred_proposal_deltas, pred_cooperative_logits = self.box_predictor(bbox_cls_feats, bbox_reg_feats)
        del box_features
...
class ConHeads(ROIHeads):
    ...
    def regression_loss(self, q, k, coord_q, coord_k, pos_ratio=0.7):
        ...
        form = (pos.sum(-1) / (pos.sum(-1) + neg.sum(-1) + 1e-6))
